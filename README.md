...
Generic **Cloudflare Tunnel + Caddy reverse proxy** one-shot deploy skill. The user supplies the tunnel token, public hostname, and listening port; the skill installs `cloudflared` + `Caddy` and wires them together. Optional client-side auth (`PROXY_API_KEY`) and upstream API-key injection (`API_KEY_ENV`).
...
- `Caddy`: listens on `LOCAL_PORT` (+ `DASHBOARD_PORT`/`EXTRA_PORT` to survive Cloudflare Dashboard port overrides). Reverse-proxies to `UPSTREAM`. Two independent auth layers:
  ...
  - `API_KEY_ENV` (optional): if set, injects `Authorization: Bearer {env.$API_KEY_ENV}` + `x-api-key` upstream so clients need zero upstream credentials.
